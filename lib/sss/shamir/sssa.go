...
func Create(minimum int, shares int, raw string) ([]string, error) {
	...
	var secret = common.SplitByteToInt([]byte(raw))
	...
	for i := range secrets {
		...
		for j := range secrets[i] {
			...
			secrets[i][j][1] = common.EvaluatePolynomial(polynomial[j], number)

			// ...add it to results...
			result[i] += common.ToBase64(secrets[i][j][0])
			result[i] += common.ToBase64(secrets[i][j][1])
		}
	}
...
func Combine(shares []string) (string, error) {
	...
	// Set constant prime
	prime := common.GetDefaultPrimeNumber()
	...
	for i := range shares {
		...
		for j := range secrets[i] {
			...
			secrets[i][j][0] = common.FromBase64(cshare[0:44])
			secrets[i][j][1] = common.FromBase64(cshare[44:])
		}
	}
	...
	for j := range secret {
		...
		for i := range secrets { // LPI sum loop
			...
			working = working.Mul(working, common.ModInverse(denominator))
	...
	return string(common.MergeIntToByte(secret)), nil
}
