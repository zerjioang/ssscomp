...
func TestPublicKeyExtraction(t *testing.T) {
	...
	public2, err := ed25519.ExtractPublicKey(wrongMessage, sig)
...
func TestSignVerify2(t *testing.T) {
	...
	sig := ed25519.Sign2(private, message)
	if !ed25519.Verify2(public, message, sig) {
		t.Errorf("valid signature rejected")
	}

	// Verification of the signature on a wrong message should fail
	wrongMessage := []byte("wrong message")
	if ed25519.Verify2(public, wrongMessage, sig) {
...
func TestDerive(t *testing.T) {
	...
	_ = ed25519.NewDerivedKeyFromSeed(seed[:], idx, salt)
}
...
func TestDerive1(t *testing.T) {
	...
	privateKey1 := ed25519.NewDerivedKeyFromSeed(seed[:], 5, []byte("Spacemesh rocks"))
...
func BenchmarkPublicKeyExtraction(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, _ = ed25519.ExtractPublicKey(message, sig)
	}
}
...
func BenchmarkVerificationExt(b *testing.B) {
	...
	message := []byte("Hello, world!")
	signature := ed25519.Sign2(priv, message)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		ed25519.Verify2(pub, message, signature)
	}
}
