...
def test():
  ...
  print "Dec(Enc(1)+Enc(0))=",d.decrypt(c1+c2)
  print "Dec(Enc(0)+Enc(1))=",d.decrypt(c2+c1)
  print "Dec(Enc(1)+Enc(1))=",d.decrypt(c1+c1)
  print "Dec(Enc(0)+Enc(0))=",d.decrypt(c2+c2)
  print "Dec(Enc(0)+Enc(0)+Enc(1))=",d.decrypt(c2+c2+c1)
  print "Dec(Enc(0)+Enc(0)+Enc(0))=",d.decrypt(c2+c2+c2)
  ...
  print "--------------"
  print "Mixed"
  print "Dec(Enc(1)*Enc(1)+Enc(0)*Enc(1))=",d.decrypt(c1*c1+c2*c1)