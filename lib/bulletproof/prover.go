...
func commitVectors(alpha *big.Int, h *ECPoint, aL []*big.Int, G []*ECPoint,
	aR []*big.Int, H []*ECPoint) *ECPoint {
	return SumPoints(
...
func commit(gamma *big.Int, h *ECPoint, v *big.Int, g *ECPoint) *ECPoint {
...
func proverCommitToT(n int, z, zz, tau1, tau2 *big.Int, aL, aR, sL, sR, Y,
	powersOfTwo []*big.Int, h, g *ECPoint) (*ECPoint, *ECPoint, *big.Int, *big.Int,
	*big.Int) {
...
// challenge.
func updateCommit(commit [32]byte, lpt, rpt *ECPoint) ([32]byte, *big.Int) {
...
func verifyInnerProductProof(i, n int, challenges []*big.Int, Ls []*ECPoint,
	Rs []*ECPoint, a []*big.Int, b []*big.Int, P, u *ECPoint, G, H []*ECPoint) bool {
	nPrime := n / 2
...
func createInnerProductProof(i, n int, commit [32]byte, G, H []*ECPoint, g, u,
	P *ECPoint, a, b []*big.Int, Ls, Rs []*ECPoint) (
	[]*ECPoint, []*ECPoint, []*big.Int, []*big.Int) {
	nPrime := n / 2
...
func (p *BulletProof) Bytes() []byte {
	...
	if _, err := buff.Write(SerializePoints(
		[]*ECPoint{p.A, p.S, p.T1, p.T2})); err != nil {
		log.Fatal(err)
	}
...
func computeHPrime(H []*ECPoint, y *big.Int) []*ECPoint {
	// y^{-n} == (y^{-1})^n
	yExpNegN := powers(Inv(y), len(H))

	// H' is a set of generators derived from H. (64)
	Hprime := make([]*ECPoint, len(H))
	for i := range H {
...
func rangeProofCreate(n int, V *ECPoint, v, gamma *big.Int, g, h *ECPoint,
	G, H []*ECPoint, nonce [32]byte, message [16]byte, powersOfTwo []*big.Int) (
	BulletProof, error) {
...
// range.
func rangeProofVerify(n int, V *ECPoint, proof BulletProof, g, h *ECPoint, G,
	H []*ECPoint) bool {
...
func NewProver(n int) *Prover {
	...
	h := ECPoint{X: curve.Gx, Y: curve.Gy}
